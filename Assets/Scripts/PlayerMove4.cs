...
public class PlayerMove4 : MonoBehaviour
{
    ...
    void Update() // 단발적인 Key 입력
    {
        ...
        {
            // if (onLadder) //5.24 점프하다 사다리 콜라이더와 충돌했을 때도 멈추길래.. -> 당장은 괜찮지만 문제가 되는 코드
            // {
            //     onLadder = false; // -> 자동 canClimbing = false -> 
            // }
        ...
        /*
                ...
                if (onLadder)
                {
                    ...
                    canClimbing = true;
        /*
                    // 사다리 위 점프 처리 5.24 -> 여기서 발전시키면 될듯? -> 위의 점프 코드 안의 onLadder = false로 해결됨?
                    if (Input.GetButtonDown("Jump"))
                    {
                        //rigid.gravityScale = 4; // 중력 다시 켜기 -> 소용 x
                        //rigid.velocity = new Vector2(rigid.velocity.x, firstJumpForce); // 위로 점프
                        if (jumpCount < maxJumpCount)
                        {
                            isJumping = true;
                            animator.SetBool("isJumping", true);
    ...
    void FixedUpdate() // 지속적인 key 입력
    {
        ...
        /*
                if (canClimbing)
                {
                    ...
                    {
                        isClimbing = false; //사다리에서 오르는 애니메이션 안하게
                        rigid.velocity = new Vector2(rigid.velocity.x, 0); // 움직임 없음, //스무스하게 움직일 수 있게
                    }
