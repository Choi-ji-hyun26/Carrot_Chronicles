...
public class PlayerMove : MonoBehaviour
{
    ...
    { 
        ...
        if(Input.GetButtonDown("Jump") && jumpCount < maxJumpCount){ 
            ...
            rigid.AddForce(Vector2.up * appliedForce, ForceMode2D.Impulse);
        ...
        // Climbing
        if(Mathf.Abs(Input.GetAxisRaw("Vertical")) > 0 && isLadder){
    ...
    void FixedUpdate() // 지속적인 key 입력
    {
       ...
       /*
        float boxWidth = GetComponent<BoxCollider2D>().size.x; // 플레이어 콜라이더의 X 크기를 사용
        float boxHeight = 0.3f; // 더 큰 높이 설정 (충돌을 잘 감지할 수 있도록)
        //Landing Platform
        if(rigid.velocity.y < 0){ //착지할때도 ray를 그리지 않게하기 위함
            RaycastHit2D rayHit = Physics2D.BoxCast(transform.position, new Vector2(boxWidth, boxHeight), 0f, Vector2.down, 0.3f, LayerMask.GetMask("Platform"));// 물리기반, 이 레이어에 해당하는 것만 스캔할 것
            // rayHit : 빔을 쏘고 오브젝트에 대한 정보
            if(rayHit.collider != null) {// 빔을 맞았을 때
                if(rayHit.distance < 0.2f) //잘 밀착했는지
                    animator.SetBool("isJumping", false);
            }
            Debug.DrawRay(transform.position, Vector3.down * 0.3f, new Color(1, 0, 0));
 ...
 */
        ...
        if (isLadder)
        {
            ...
            //점프 중 사다리에서 멈추면 중력 값이 0이 아니되게 하기 위함 
            ...
            rigid.velocity = new Vector2(rigid.velocity.x, vertical * maxSpeed);
        }
        else 
        {
    ...
    void OnCollisionEnter2D(Collision2D collision) {
        if(collision.gameObject.tag == "Enemy"){
            //Attack
            if(rigid.velocity.y < -0.01f && transform.position.y > collision.transform.position.y) // 점프공격 낙하, 위치 파악 + 뒤에 조건 불필요한가?
            {  
